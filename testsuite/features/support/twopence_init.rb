...
# TwoPence definitions (network nodes)
module TwoPenceLib
  # rubocop:disable Metrics/PerceivedComplexity, Metrics/CyclomaticComplexity
  # Warnings
  def self.warning_missing_nodes
    # Initialize SSH targets from environment variables
    raise(KeyError, 'Server IP address or domain name variable empty') if ENV['SERVER'].nil?
  ...
  # Define common twopence nodes
  def self.twopence_init_common_nodes
    ...
    if ENV['VIRTHOST_KVM_URL'] && ENV['VIRTHOST_KVM_PASSWORD']
      $kvm_server = twopence_init("ssh:#{ENV['VIRTHOST_KVM_URL']}")
    end
    if ENV['VIRTHOST_XEN_URL'] && ENV['VIRTHOST_XEN_PASSWORD']
      $xen_server = twopence_init("ssh:#{ENV['VIRTHOST_XEN_URL']}")
    end
  ...
  # Define other TwoPence nodes depending on type of environment
  def self.twopence_init_other_nodes
    if $build_validation
  ...
  # Initialize nodes extending with LavandaBasic module
  def self.extend_nodes
    $nodes.each do |node|
      next if node.nil?
      ...
      if code.nonzero?
        raise(ScriptError, "Cannot connect to get hostname for '#{$named_nodes[node.hash]}'. Error: #{code}")
      end
      ...
      raise(ScriptError, "Cannot connect to get FQDN for '#{$named_nodes[node.hash]}'. Error: #{code}") if code.nonzero?
      ...
      unless $build_validation
        puts("Host '#{$named_nodes[node.hash]}' is alive with hostname #{hostname.strip} and FQDN #{fqdn.strip}")
      end
    end
  end

  # rubocop:disable Metrics/MethodLength
  # Generate two hash table of node by host and viceversa
  def self.hash_tables_host_node
    $node_by_host = {
      'localhost' => $localhost,
      'server' => $server,
      'proxy' => $proxy,
    ...
    $node_by_host.each do |host, node|
      ...
      [host, node].each do |it|
        raise(ArgumentError, ">>> Either host '#{host}' of node '#{node}' is empty.  Please check") if it == ''
      end
  ...
  def self.init_ip_per_node
    $nodes.each do |node|
      ...
      raise(ArgumentError, "Cannot resolve host for node: '#{node.hostname}'") if host.nil? || host == ''
  ...
  def self.initialize_nodes
    warning_missing_nodes
    # Preserve FQDN before initialization
    $named_nodes = {}
    twopence_init_common_nodes
    twopence_init_other_nodes
    extend_nodes
    hash_tables_host_node
    init_ip_per_node
    # Define the kind of product installed on the server node
    $product = CommonLib.product
  end

  # Initialize a TwoPence class for a provided target (domain or IP)
  def self.twopence_init(target)
  ...
  def get_target(host)
    ...
    raise(ArgumentError, 'Invalid target') if node.nil?
    ...
    if host == 'pxeboot_minion'
      ...
      system_name =
        output.split.find do |word|
          word =~ /example.Intel-Genuine-None-/ || word =~ /example.pxeboot-/ || \
            word =~ /example.Intel/ || word =~ /pxeboot-/
        end
  ...
  # Extract Public IP
  def self.client_public_ip(host)
    node = $node_by_host[host]
    raise(ArgumentError, "Cannot resolve node for host '#{host}'") if node.nil?
    ...
    output, code = node.run("ip address show dev #{CommonLib.get_interface_slot(host)} | grep 'inet '")
    raise(ScriptError, 'Cannot resolve public ip') unless code.zero?
