...
module ClientStack
  ...
  def check_restart(host, node, time_out)
    ...
    repeat_until_timeout(timeout: time_out, message: "machine didn't come up") do
      _out, code = node.run('ls', fatal: false, timeout: 10)
      puts("machine: #{host} ssh is up"); break if code.zero?
    end
  end

  # Extract the OS family
  # We get these data decoding the values in '/etc/os-release'
  def get_os_family(node)
    os_family_raw, code = node.run('grep "^ID=" /etc/os-release', fatal: false)
  ...
  def get_os_version(node)
    os_family = get_os_family(node)

    os_version_raw, code = node.run('grep "^VERSION_ID=" /etc/os-release', fatal: false)
    return nil, nil unless code.zero?
  ...
  def get_gpg_keys(node, target)
    ...
    when /^sles/
      ...
      gpg_keys, _code = target.run("cd /srv/www/htdocs/pub/ && ls -1 sle#{os_version}*", fatal: false)
    when /^centos/
      gpg_keys, _code = target.run("cd /srv/www/htdocs/pub/ && ls -1 #{os_family}#{os_version}* res*", fatal: false)
    else
      gpg_keys, _code = target.run("cd /srv/www/htdocs/pub/ && ls -1 #{os_family}*", fatal: false)
    end
  ...
  def sle11family?(node)
    _out, code = node.run('pidof systemd', fatal: false)
    code.nonzero?
  end
