...
module CommonLib
  ...
  def compute_list_to_leave_running
    ...
    [$minion, $build_host, $sshminion].each do |node|
      ...
      raise(StandardError, "Can't build list of reposyncs to leave running") unless %w[
        12-SP4
        12-SP5
        15-SP2
        15-SP3
  ...
  def self.product
    _product_raw, code = $server.run('rpm -q patterns-uyuni_server', fatal: false)
    return 'Uyuni' if code.zero?

    _product_raw, code = $server.run('rpm -q patterns-suma_server', fatal: false)
  ...
  def self.get_interface_slot(host)
    case host
  ...
  def repeat_until_timeout(timeout: DEFAULT_TIMEOUT, retries: nil, message: nil, report_result: false)
    ...
    Timeout.timeout(timeout) do
      ...
      raise(TimeoutError, "Giving up after #{attempts} attempts#{detail}") if attempts == retries
    ...
    raise(
      TimeoutError,
      "Timeout after #{timeout} seconds (Timeout.timeout)#{format_detail(message, last_result, report_result)}"
    )
  end
  ...
  def extract_logs_from_node(node)
    ...
    node.run('journalctl > /var/log/messages', fatal: false)
