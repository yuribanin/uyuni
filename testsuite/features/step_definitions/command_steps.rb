...
Then(/^"([^"]*)" should have a FQDN$/) do |host|
  ...
  result, return_code = node.run('hostname -f', fatal: false)
  result.delete!("\n")
...
Then(/^reverse resolution should work for "([^"]*)"$/) do |host|
  node = get_target(host)
  result, return_code = node.run("getent hosts #{node.ip}", fatal: false)
  result.delete!("\n")
  raise ScriptError, 'cannot do reverse resolution' unless return_code.zero?
  unless result.include?(node.full_hostname)
    raise ScriptError, "reverse resolution returned #{result}, expected to see #{node.full_hostname}"
  end
end
...
When(/^I delete these channels with spacewalk-remove-channel:$/) do |table|
  ...
  $command_output, _return_code = $server.run(channels_cmd, fatal: false)
end
...
Then(/^spacewalk-channel should fail adding "([^"]*)"$/) do |invalid_channel|
  ...
  $command_output, code = $client.run(command, fatal: false)
...
When(/^I wait until all spacewalk-repo-sync finished$/) do
  ...
  while reposync_not_running_streak <= 30
    command_output, _code = $server.run('ps axo pid,cmd | grep spacewalk-repo-sync | grep -v grep', fatal: false)
...
When(/^I kill all running spacewalk-repo-sync, excepted the ones needed to bootstrap$/) do
  ...
  while reposync_not_running_streak <= 30 && reposync_left_running_streak <= 7200
    ...
    pid = process.split[0]
    $server.run("kill #{pid}", fatal: false)
    $stdout.puts("Reposync of channel #{channel} killed")
  end
end

Then(/^the reposync logs should not report errors$/) do
  result, code = $server.run('grep "ERROR:" /var/log/rhn/reposync/*.log', fatal: false)
...
When(/^I wait until the channel "([^"]*)" has been synced$/) do |channel|
  repeat_until_timeout(timeout: 7200, message: 'Channel not fully synced') do
    _result, code = $server.run("test -f /var/cache/rhn/repodata/#{channel}/repomd.xml", fatal: false)
    break if code.zero?
...
Then(/^file "([^"]*)" should contain "([^"]*)" on server$/) do |file, content|
  ...
  raise ScriptError, "'#{content}' not found in file #{file}" if output[:stdout] !~ /#{content}/
...
Then(/^file "([^"]*)" should not contain "([^"]*)" on server$/) do |file, content|
  ...
  raise ScriptError, "'#{content}' found in file #{file}" if output[:stdout] != "notfound\n"
...
When(/^I clean the search index on the server$/) do
  ...
  raise ScriptError, 'The output includes an error log' if output[:stdout].include?('ERROR')
end
...
Then(/^I wait until mgr-sync refresh is finished$/) do
  ...
  repeat_until_timeout(timeout: 1800, message: "'mgr-sync refresh' did not finish") do
    result, _code = $server.run(cmd, fatal: false)
...
Then(/^service "([^"]*)" is enabled on "([^"]*)"$/) do |service, host|
  node = get_target(host)
  output, _code = node.run("systemctl is-enabled '#{service}'", fatal: false)
  output = output.split(/\n+/)[-1]
...
Then(/^service "([^"]*)" is active on "([^"]*)"$/) do |service, host|
  node = get_target(host)
  output, _code = node.run("systemctl is-active '#{service}'", fatal: false)
  output = output.split(/\n+/)[-1]
...
Then(/^service or socket "([^"]*)" is enabled on "([^"]*)"$/) do |name, host|
  node = get_target(host)
  output_service, _code_service = node.run("systemctl is-enabled '#{name}'", fatal: false)
  output_service = output_service.split(/\n+/)[-1]
  output_socket, _code_socket = node.run(" systemctl is-enabled '#{name}.socket'", fatal: false)
  output_socket = output_socket.split(/\n+/)[-1]
  raise if (output_service != 'enabled') && (output_socket != 'enabled')
end

Then(/^service or socket "([^"]*)" is active on "([^"]*)"$/) do |name, host|
  node = get_target(host)
  output_service, _code_service = node.run("systemctl is-active '#{name}'", fatal: false)
  output_service = output_service.split(/\n+/)[-1]
  output_socket, _code_socket = node.run(" systemctl is-active '#{name}.socket'", fatal: false)
  output_socket = output_socket.split(/\n+/)[-1]
  raise if (output_service != 'active') && (output_socket != 'active')
end

Then(/^socket "([^"]*)" is enabled on "([^"]*)"$/) do |service, host|
  node = get_target(host)
  output, _code = node.run("systemctl is-enabled '#{service}.socket'", fatal: false)
  output = output.split(/\n+/)[-1]
  raise ScriptError, "Service #{service} not enabled" if output != 'enabled'
end
...
Then(/^socket "([^"]*)" is active on "([^"]*)"$/) do |service, host|
  node = get_target(host)
  output, _code = node.run("systemctl is-active '#{service}.socket'", fatal: false)
  output = output.split(/\n+/)[-1]
  raise ScriptError, "Service #{service} not active" if output != 'active'
end
...
When(/^I run "([^"]*)" on "([^"]*)" without error control$/) do |cmd, host|
  node = get_target(host)
  _out, $fail_code = node.run(cmd, fatal: false)
end
...
When(/^I (install|remove) OpenSCAP dependencies (on|from) "([^"]*)"$/) do |action, where, host|
  ...
  when /^ubuntu/
    ...
    raise(ArgumentError, "The node #{node.hostname} has not a supported OS Family")
  end
...
When(/^I wait until the package "(.*?)" has been cached on this "(.*?)"$/) do |pkg_name, host|
  ...
  repeat_until_timeout(message: "Package #{pkg_name} was not cached") do
    _result, return_code = node.run(cmd, fatal: false)
...
Then(/^terminal "([^"]*)" should have got a retail network IP address$/) do |host|
  ...
  unless return_code.zero? && output.include?(net_prefix)
    raise ScriptError, "Terminal #{host} did not get an address on eth1: #{output}"
  end
end
...
Then(/^name resolution should work on terminal "([^"]*)"$/) do |host|
  ...
  %w[proxy.example.org dns.google.com].each do |dest|
    ...
    unless return_code.zero?
      raise(ScriptError, "Direct name resolution of #{dest} on terminal #{host} doesn't work: #{output}")
    end
  ...
  [client, '8.8.8.8'].each do |dest|
    output, return_code = node.run("host #{dest}", fatal: false)
    unless return_code.zero?
      raise ScriptError, "Reverse name resolution of #{dest} on terminal #{host} doesn't work: #{output}"
    end
...
Then(/^The metadata buildtime from package "(.*?)" match the one in the rpm on "(.*?)"$/) do |pkg, host|
  ...
  raise ScriptError, "Command failed: #{cmd}" unless return_code.zero?
  unless metadata_buildtime == rpm_buildtime
    raise ScriptError, "Wrong buildtime in metadata: #{metadata_buildtime} != #{rpm_buildtime}"
  end
end
...
When(/^I create ([^ ]*) virtual storage pool on "([^"]*)"$/) do |pool_name, host|
  ...
  _output, code = node.run("virsh pool-dumpxml #{pool_name}", fatal: false)
  unless code.zero?
    node.run("echo -e \"#{pool_def}\" >/tmp/#{pool_name}.xml && virsh pool-define /tmp/#{pool_name}.xml")
  end
  node.run("mkdir -p /var/lib/libvirt/images/#{pool_name}")

  # Ensure the pool is started
  node.run("virsh pool-start #{pool_name}", fatal: false)
end
...
When(/^I delete ([^ ]*) virtual storage pool on "([^"]*)"((?: without error control)?)$/) do |pool_name, host, error_control|
  ...
  if pool_name.start_with?('test-')
    step %(I run "rm -rf /var/lib/libvirt/images/#{pool_name}" on "#{host}"#{error_control})
  end
end
...
When(/^I delete all "([^"]*)" volumes from "([^"]*)" pool on "([^"]*)" without error control$/) do |volumes, pool, host|
  ...
  output, _code = node.run(
    "virsh vol-list #{pool} | sed -n -e 's/^[[:space:]]*\([^[:space:]]\+\).*$/\1/;/#{volumes}/p'", false
  )
  output.each_line { |volume| node.run("virsh vol-delete #{volume} #{pool}", fatal: false) }
end
...
When(/^spacecmd should show packages "([^"]*)" installed on "([^"]*)"$/) do |packages, client|
  ...
  result, _code = $server.run(command, fatal: false)
