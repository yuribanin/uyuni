...
Then(/^"(.*?)" is locked on this client$/) do |pkg|
  ...
  raise(ScriptError, "File already exist: #{zypp_lock_file}") unless file_exists?($client, zypp_lock_file)

  command = "zypper locks  --solvables | grep #{pkg}"
  $client.run(command, fatal: true, timeout: 600, user: 'root')
end
...
Then(/^"(.*?)" is unlocked on this client$/) do |pkg|
  zypp_lock_file = '/etc/zypp/locks'
  raise(ScriptError, "File #{zypp_lock_file} not found") unless file_exists?($client, zypp_lock_file)
...
Then(/^only packages "(.*?)" are reported as pending to be unlocked$/) do |pkgs|
  ...
  if matches.size != pkgs.size
    raise(StandardError, "Matches count #{matches.size} is different than packages count #{pkgs.size}")
  end
end
