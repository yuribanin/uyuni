...
Then(/^the image "([^"]*)" with version "([^"]*)" doesn't exist via XML-RPC calls$/) do |image_non_exist, version|
  ...
  images_list = $cont_op.list_images
  images_list.each do |element|
    if element['name'] == image_non_exist && element['version'] == version.strip
      raise(ScriptError, "#{image_non_exist} should not exist anymore")
    end
  end
end
...
When(/^I list image store types and image stores via XML-RPC$/) do
  ...
  unless store_typ.length == 2
    raise(ScriptError, 'We have only type support for Registry and OS Image Store type! New method added?! please update the tests')
  end
  unless store_typ[0]['label'] == 'registry'
    raise(ScriptError, "imagestore label type should be 'registry' but is #{store_typ[0]['label']}")
  end
  unless store_typ[1]['label'] == 'os_image'
    raise(ScriptError, "imagestore label type should be 'os_image' but is #{store_typ[1]['label']}")
  end

  registry_list = $cont_op.list_image_stores
  unless registry_list[0]['label'] == 'galaxy-registry'
    raise(ScriptError, "Label #{registry_list[0]['label']} is different than 'galaxy-registry'")
  end
  unless registry_list[0]['uri'] == $no_auth_registry.to_s
    raise(ScriptError, "URI #{registry_list[0]['uri']} is different than '#{$no_auth_registry}'")
  end
end
...
When(/^I create and delete profile custom values via XML-RPC$/) do
  ...
  unless pro_det['arancio'] == 'arancia xmlrpc tests'
    raise(ScriptError, "setting custom profile value failed: #{pro_det['arancio']} != 'arancia xmlrpc tests'")
  end
...
When(/^I list image profiles via XML-RPC$/) do
  ...
  unless imagelabel[0]['label'] == 'fakeone'
    raise(ScriptError, "label of container should be fakeone! #{imagelabel[0]['label']} != 'fakeone'")
  end
end
...
When(/^I set and get profile details via XML-RPC$/) do
  ...
  unless cont_detail['imageType'] == 'dockerfile'
    raise(ScriptError, "imagetype test fail! #{cont_detail['imageType']} != 'dockerfile'")
  end
