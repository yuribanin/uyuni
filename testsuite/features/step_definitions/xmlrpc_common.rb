...
When(/^I call listActivationKeys I should get some$/) do
  raise if @activation_key_api.activation_keys_count < 1
end
...
When(/^I delete channel "([^"]*)" via XML-RPC((?: without error control)?)$/) do |channel, error_control|
  ...
  if error_control.empty?
    raise(XMLRPC::FaultException, "Error delete_channels: XML-RPC failure, code #{e.faultCode}: #{e.faultString}")
  end
end
