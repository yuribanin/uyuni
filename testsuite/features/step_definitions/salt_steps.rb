...
When(/^I stop salt-minion on "(.*?)"$/) do |minion|
  ...
  node.run('rcsalt-minion stop', fatal: false) if minion == 'sle_minion'
  node.run('systemctl stop salt-minion', fatal: false) if %w[
    ceos_minion
    ubuntu_minion
    kvm_server
    xen_server
...
When(/^I start salt-minion on "(.*?)"$/) do |minion|
  node = get_target(minion)
  node.run('rcsalt-minion restart', fatal: false) if minion == 'sle_minion'
  node.run('systemctl restart salt-minion', fatal: false) if %w[
    ceos_minion
    ubuntu_minion
    kvm_server
    xen_server
...
When(/^I wait at most (\d+) seconds until Salt master sees "([^"]*)" as "([^"]*)"$/) do |key_timeout, minion, key_type|
  ...
  repeat_until_timeout(
    ...
    unless system_name.empty?
      output, return_code = $server.run(cmd, fatal: false)
...
When(/^I delete "([^"]*)" key in the Salt master$/) do |host|
  ...
  $output, _code = $server.run("salt-key -y -d #{system_name}", fatal: false)
end
...
When(/^I ([^ ]*) the "([^"]*)" formula$/) do |action, formula|
  ...
  if all(:xpath, xpath_query, wait: DEFAULT_TIMEOUT).any?
    unless find(:xpath, xpath_query, wait: DEFAULT_TIMEOUT).click
      raise(StandardError, "xpath: #{xpath_query} not found")
    end
  else
...
Then(/^the keymap on "([^"]*)" should be "([^"]*)"$/) do |minion, keymap|
  ...
  unless output.strip == "KEYMAP=#{keymap}"
    raise(ScriptError, "The keymap #{keymap} is different to the output: #{output.strip}")
  end
end
...
Then(/^the language on "([^"]*)" should be "([^"]*)"$/) do |minion, language|
  ...
  unless output.strip == "RC_LANG=\"#{language}\""
    ...
    unless output.strip == "LANG=#{language}"
      raise(ScriptError, "The language #{language} is different to the output: #{output.strip}")
    end
  end
end
...
When(/^I uninstall Salt packages from "(.*?)"$/) do |host|
  ...
  if %w[sle_minion ssh_minion sle_client].include?(host)
    target.run('test -e /usr/bin/zypper && zypper --non-interactive remove -y salt salt-minion', fatal: false)
  elsif %w[ceos_minion].include?(host)
    target.run('test -e /usr/bin/yum && yum -y remove salt salt-minion', fatal: false)
  elsif %w[ubuntu_minion].include?(host)
    target.run('test -e /usr/bin/apt && apt -y remove salt-common salt-minion', fatal: false)
  end
end
...
When(/^I install Salt packages from "(.*?)"$/) do |host|
  target = get_target(host)
  if %w[sle_minion ssh_minion sle_client].include?(host)
    target.run('test -e /usr/bin/zypper && zypper --non-interactive install -y salt salt-minion', fatal: false)
  elsif %w[ceos_minion].include?(host)
    target.run('test -e /usr/bin/yum && yum -y install salt salt-minion', fatal: false)
  elsif %w[ubuntu_minion].include?(host)
    target.run('test -e /usr/bin/apt && apt -y install salt-common salt-minion', fatal: false)
  end
end
...
When(/^I perform a full salt minion cleanup on "([^"]*)"$/) do |host|
  ...
  if host.include?('ceos')
    node.run('yum -y remove --setopt=clean_requirements_on_remove=1 salt salt-minion', fatal: false)
  elsif host.include?('ubuntu') || host.include?('debian')
    node.run(
      'apt-get --assume-yes remove salt-common salt-minion && apt-get purge salt-common salt-minion && apt-get autoremove', fatal: false
    )
  else
    node.run('zypper --non-interactive remove --clean-deps -y salt salt-minion spacewalk-proxy-salt', fatal: false)
  end
  node.run('rm -Rf /root/salt /var/cache/salt/minion /var/run/salt /var/log/salt /etc/salt /var/tmp/.root*', fatal: false)
