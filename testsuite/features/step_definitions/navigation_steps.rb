...
Given(/^I access the host the first time$/) do
  visit Capybara.app_host
  unless has_content?("Create #{product} Administrator")
    raise(StandardError, "Text 'Create #{product} Administrator' not found")
  end
end
...
Then(/^I wait until table row for "([^"]*)" contains "([^"]*)"$/) do |arg1, arg2|
  ...
  within(:xpath, xpath_query) do
    unless has_content?(arg2, wait: DEFAULT_TIMEOUT)
      raise(StandardError, "xpath: #{xpath_query} has no content #{arg2}")
    end
  end
end
...
Then(/^the table row for "([^"]*)" should( not)? contain "([^"]*)" icon$/) do |row, should_not, icon|
  ...
  within(:xpath, xpath_query) do
    if should_not
      unless has_no_css?(content_selector, wait: DEFAULT_TIMEOUT)
        raise(StandardError, "xpath: #{xpath_query} has no icon #{icon}")
      end
    else
      unless has_css?(content_selector, wait: DEFAULT_TIMEOUT)
        raise(StandardError, "xpath: #{xpath_query} has no icon #{icon}")
      end
    end
  end
...
Then(/^I should see a "([^"]*)" or "([^"]*)" text in element "([^"]*)"$/) do |text1, text2, element|
  within(:xpath, "//div[@id=\"#{element}\" or @class=\"#{element}\"]") do
    unless has_content?(text1) || has_content?(text2)
      raise(StandardError, "Texts #{text1} and #{text2} not found in #{element}")
    end
  end
end
...
Then(/^I should see (\d+) "([^"]*)" fields in "([^"]*)" form$/) do |count, name, id|
  within(:xpath, "//form[@id=\"#{id}\" or  @name=\"#{id}\"]") do
    unless has_field?(name, count: count.to_i)
      raise(StandardError, "#{id} form has not #{count} fields with name #{name}")
    end
  end
end
